...
1. 提供一个设置页面，里面分为几个 section，每个Section都是一个表单。
2. section 从上到到下排列，每个 section 都会在表单前显示自己的标题。
3. 表单无需“保存”按钮，表单字段修改后，自动保存到数据库

## 4.2 LLM API

设置中的一个 section，用于设置用户像LLM API发送请求的设置。

### 4.2.1 数据模型

1. 添加数据表 LLMSettings 和对应的数据对象
2. LLMSetting 包括以下个字段：
   1. ID：用户设置条目ID
   2. API URL：LLM API的URL
   3. API KEY：用户个人的 LLM API Key
   4. Model：进行 LLM 请求时使用的模型名称
3. 表单中的设置保存为 LLMSettings 的条目。目前先只保存一条，后续都是更新这一条。

### 4.2.2 界面交互

1. 表单中提供以下控件：
   - 单行文本输入框，允许用户输入OpenAI兼容标准的 API URL
   - 密码输入框/单行文本输入框，允许用户输入 API KEY。输入框右侧提供一个”眼睛“按钮，可以在查看KEY（普通文本框）和隐藏KEY（密码框）之间切换
   - 一个 <Select> 组件，允许用户选择当前API下可用的AI模型。
      - 未保存任何选中的模型，且模型列表为空时，显示“获取模型列表以选择模型”的 placeholder 文字
      - 未保存任何选中模型，但模型列表有值时，默认选中第一个
      - 有保存的选中模型时，无论模型列表是否有值，都在SELECT里显示当前选中的模型
2. <Select> 右侧有一个”获取模型“的按钮，点击后使用当前API和KEY获取模型。
   - 如果API或KEY未填写，则按钮置灰
   - 获取模型过程中按钮变为”取消“
   - 点击”取消“后停止之前获取模型的请求
   - 模型获取成功后，若当前没有保存过的选中模型，则自动选中列表中第一个